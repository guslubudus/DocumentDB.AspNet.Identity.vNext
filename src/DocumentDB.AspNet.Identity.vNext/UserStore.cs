    ...
    public class UserStore<TUser> :
       ...
       where TUser : IdentityUser {
        ...
        private async Task UpdateUserAsync( TUser user ) {
            ...
            if ( doc != null ) {
                foreach ( var pi in GetPropertyInfoCache( ) ) {
                    doc.SetPropertyValue( pi.Name, pi.GetValue( user ) );
                }
                await client.ReplaceDocumentAsync( (dynamic)doc );
            }
        }
        private static PropertyInfo[] piCache;
        private static PropertyInfo[] GetPropertyInfoCache( ) {
            if ( piCache == null ) {
                piCache = typeof( TUser ).GetPublicProperties( typeof( IUser<string> ) );
            }
            return piCache;
        }
